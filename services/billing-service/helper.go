...
import (
	...
	"github.com/fadliarz/distributed-faas/services/billing-service/config"
	"github.com/fadliarz/distributed-faas/services/billing-service/domain/application-service"
	"github.com/fadliarz/distributed-faas/services/billing-service/infrastructure/repository"
	"github.com/fadliarz/distributed-faas/services/billing-service/rpc"
...
func loadEnv(config *Config) {
	...
	if port := os.Getenv("BILLING_SERVICE_PORT"); port != "" {
		config.Port = ":" + port
...
func setupCommandHandler(repositoryManager *RepositoryManager) *application.CommandHandler {
	// Application Service
	applicationService := application.NewBillingApplicationService(
		application.NewBillingApplicationServiceRepositoryManager(repositoryManager.Billing),
	)
...
func setupGRPCServer(config *Config, dependencies *Dependencies) (*grpc.Server, net.Listener, error) {
	...
	billingServer := rpc.NewBillingServiceServer(dependencies.handler)
	billingServer.Register(grpcServer)
