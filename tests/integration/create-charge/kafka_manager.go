...
func (km *KafkaManager) SetupConsumers(bootstrapServer string) error {
	if err := km.createTopicsIfNotExist(bootstrapServer, []string{km.config.KafkaConfig.Topics.Charge}); err != nil {
		return fmt.Errorf("failed to create topics: %w", err)
	}

	if err := km.setupChargeConsumer(bootstrapServer); err != nil {
		return fmt.Errorf("failed to setup charge consumer: %w", err)
	}
...
func (km *KafkaManager) setupChargeConsumer(bootstrapServer string) error {
	var err error

	km.Consumers.Charge, err = kafka.NewConsumer(&kafka.ConfigMap{
		"bootstrap.servers": bootstrapServer,
		"group.id":          km.config.KafkaConfig.ConsumerGroups.Charge,
		"auto.offset.reset": "earliest",
	})
	if err != nil {
		return fmt.Errorf("failed to create charge consumer: %w", err)
	}

	err = km.Consumers.Charge.Subscribe(km.config.KafkaConfig.Topics.Charge, nil)
	if err != nil {
		return fmt.Errorf("failed to subscribe to charge topic: %w", err)
	}
