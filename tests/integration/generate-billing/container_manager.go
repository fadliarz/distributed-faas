...
func (cm *ContainerManager) setupDebeziumConnectors() error {
	err := cm.setupCronDebeziumConnector()
	if err != nil {
		return fmt.Errorf("failed to setup cron Debezium connector: %w", err)
	}

	return nil
}

func (cm *ContainerManager) setupCronDebeziumConnector() error {
	...
	configJSON := fmt.Sprintf(`{
		...
		cm.config.DebeziumConfig.CronConnectorName,
		cm.config.GetMongoConnectionString(),
		cm.config.MongoConfig.CronDatabase,
		fmt.Sprintf("%s.%s", cm.config.MongoConfig.CronDatabase, cm.config.MongoConfig.CronCollection),
	)
	...
	for attempt := 1; attempt <= cm.config.DebeziumConfig.MaxRetries; attempt++ {
		log.Debug().Msgf("[%s] Attempting to create Debezium connector (attempt %d/%d)", cm.config.DebeziumConfig.CronConnectorName, attempt, cm.config.DebeziumConfig.MaxRetries)

		// Create HTTP request
		req, err := http.NewRequest("POST", endpoint, strings.NewReader(configJSON))
		if err != nil {
			return fmt.Errorf("[%s] failed to create HTTP request: %w", cm.config.DebeziumConfig.CronConnectorName, err)
		}
		...
		// Handle response
		if err != nil {
			log.Error().Err(err).Msgf("[%s] Failed to create Debezium connector", cm.config.DebeziumConfig.CronConnectorName)
		}

		if err == nil && res.StatusCode >= 200 && res.StatusCode < 300 {
			log.Debug().Msgf("[%s] Debezium connector created successfully on attempt %d", cm.config.DebeziumConfig.CronConnectorName, attempt)

			res.Body.Close()
			return cm.waitForCronConnectorReady(fmt.Sprintf("%s/%s/status", endpoint, cm.config.DebeziumConfig.CronConnectorName))
		}
		...
		if res != nil {
			...
			log.Error().Msgf("[%s] Failed to create Debezium connector, status code: %d, response body: %s", cm.config.DebeziumConfig.CronConnectorName, res.StatusCode, body)
		}
	...
	return fmt.Errorf("[%s] failed to create Debezium connector after %d attempts", cm.config.DebeziumConfig.CronConnectorName, cm.config.DebeziumConfig.MaxRetries)
}

func (cm *ContainerManager) waitForCronConnectorReady(endpoint string) error {
	deadline := time.Now().Add(cm.config.DebeziumConfig.ReadyTimeout)

	for time.Now().Before(deadline) {
		log.Debug().Msgf("[%s] Checking Debezium connector status", cm.config.DebeziumConfig.CronConnectorName)

		res, err := http.Get(endpoint)

		if err != nil {
			log.Error().Err(err).Msgf("[%s] Failed to get Debezium connector status", cm.config.DebeziumConfig.CronConnectorName)
		...
		res.Body.Close()
		if err != nil {
			log.Error().Err(err).Msgf("[%s] Failed to read Debezium connector response body", cm.config.DebeziumConfig.CronConnectorName)
			time.Sleep(cm.config.DebeziumConfig.ReadyInterval)
			continue
		}

		if strings.Contains(string(body), `"state":"RUNNING"`) {
			log.Debug().Msgf("[%s] Debezium connector is ready", cm.config.DebeziumConfig.CronConnectorName)
			return nil
		}
