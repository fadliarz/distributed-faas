...
func (km *KafkaManager) SetupProducers(bootstrapServer string) error {
	if err := km.setupCronProducer(bootstrapServer); err != nil {
		return fmt.Errorf("failed to setup cron producer: %w", err)
	}

	return nil
}

func (km *KafkaManager) setupCronProducer(bootstrapServer string) error {
	var err error

	km.Producers.Cron, err = kafka.NewProducer(&kafka.ConfigMap{
		"bootstrap.servers": bootstrapServer,
